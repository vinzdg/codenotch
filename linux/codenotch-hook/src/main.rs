...
fn config_dir() -> Option<std::path::PathBuf> {
    #[cfg(windows)]
    {
        std::env::var_os("APPDATA").map(|a| std::path::PathBuf::from(a).join("codenotch"))
    }
    #[cfg(not(windows))]
    {
        if let Some(xdg) = std::env::var_os("XDG_CONFIG_HOME") {
            return Some(std::path::PathBuf::from(xdg).join("codenotch"));
        }
        std::env::var_os("HOME").map(|h| std::path::PathBuf::from(h).join(".config").join("codenotch"))
    }
}
...
fn find_main() -> Option<std::path::PathBuf> {
    ...
    if let Some(dir) = config_dir() {
        if let Ok(txt) = std::fs::read_to_string(dir.join("launch-path")) {

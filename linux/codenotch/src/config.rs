...
/// Sidecar the hook and XDG autostart read: APPIMAGE when packed, otherwise this process's exe.
pub fn launch_path_file() -> PathBuf {
    config_path().with_file_name("launch-path")
}

pub fn persist_launch_path() {
...
pub fn launch_path() -> Option<PathBuf> {
    let from_file = std::fs::read_to_string(launch_path_file())
        .ok()
        .map(|s| s.trim().to_string())
        .filter(|s| !s.is_empty())
        .map(PathBuf::from)
        .filter(|p| p.is_file());
    from_file.or_else(|| std::env::current_exe().ok())
}

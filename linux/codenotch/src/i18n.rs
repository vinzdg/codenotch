...
fn lang_from_tag(name: &str) -> Option<&'static str> {
    let name = name.to_ascii_lowercase();
    if name.starts_with("zh") {
        Some("zh")
    } else if name.starts_with("ja") {
        Some("ja")
    } else if name.starts_with("ko") {
        Some("ko")
    } else if name.starts_with("ru") {
        Some("ru")
...
pub fn resolve_auto() -> &'static str {
    ...
    unsafe {
        ...
        if n > 0 {
            ...
            if let Some(lang) = lang_from_tag(&name) {
                return lang;
            }
        }
    }
    #[cfg(not(windows))]
    {
        for key in ["LC_ALL", "LC_MESSAGES", "LANG"] {
            if let Ok(val) = std::env::var(key) {
                if let Some(lang) = lang_from_tag(&val) {
                    return lang;
                }
            }
        }
...
fn time_format() -> Option<String> {
    ...
    #[cfg(not(windows))]
    {
        let loc = std::env::var("LC_TIME")
            .or_else(|_| std::env::var("LC_ALL"))
            .or_else(|_| std::env::var("LANG"))
            .unwrap_or_default()
            .to_ascii_lowercase();
        if loc.starts_with("en_us") || loc.starts_with("en_ph") {
            return Some("hh:mm tt".into());
        }
        if !loc.is_empty() && loc != "c" && loc != "posix" {
            return Some("HH:mm".into());
        }
    }
    None
}
...
mod tests {
    ...
    #[test]
    fn lang_from_posix_and_bcp47_tags() {
        assert_eq!(super::lang_from_tag("pt_BR.UTF-8"), None);
        assert_eq!(super::lang_from_tag("zh_CN.UTF-8"), Some("zh"));
        assert_eq!(super::lang_from_tag("ja_JP"), Some("ja"));
        assert_eq!(super::lang_from_tag("uk_UA.UTF-8"), Some("uk"));
    }

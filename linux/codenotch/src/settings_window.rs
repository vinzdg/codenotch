...
fn open_now(app: &AppHandle) {
    ...
    // gives it Windows 11's rounded corners. On Linux a frameless WebKitGTK window often paints
    // blank white, so that build keeps the title bar and an opaque surface.
    ...
    #[cfg(not(windows))]
    {
        builder = builder
            ...
            .shadow(false)
            .transparent(false)
            .background_color(tauri::window::Color(0x2b, 0x2b, 0x2e, 255));
    }
    let loaded = crate::new_linux_load_flag();
    builder = builder.on_page_load({
        let loaded = loaded.clone();
        move |_w, payload| {
            crate::applog(&format!("settings page {:?}: {}", payload.event(), payload.url()));
    ...
    match builder.build() {
        Ok(w) => {
            crate::tune_linux_webview(&w, "settings", crate::SETTINGS_HTML, loaded);
            let _ = w.show();
            let _ = w.set_focus();
        }
...
pub fn open_author_page() {
    crate::open::url("https://x.com/hivinz_");
}

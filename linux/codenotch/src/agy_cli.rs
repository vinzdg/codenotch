...
/// Discover the official Antigravity CLI. Windows looks for `agy.exe`; Linux for `agy`.
pub fn find_agy() -> Option<PathBuf> {
    if let Some(local) = dirs::data_local_dir() {
        for name in ["agy", "agy.exe"] {
            let candidate = local.join("agy").join("bin").join(name);
...
fn run_cmd_conpty(
    ...
    let mut child = cmd.spawn().map_err(|e| format!("Cannot start Antigravity CLI: {e}"))?;
    ...
    loop {
        match child.try_wait() {
            Ok(Some(status)) => {
                let raw_bytes = reader.join().map_err(|_| "CLI output reader thread panicked".to_string())?;
                if !status.success() {
                    let code = status.code().unwrap_or(-1);
                    return Err(format!("Antigravity CLI failed with exit code {code}"));
                }
                if raw_bytes.len() > 65536 {
                    return Err("CLI quota output is too large".into());
                }
                let text = String::from_utf8_lossy(&raw_bytes);
                return Ok(sanitize_terminal_output(&text));
            }
            ...
            Err(e) => return Err(format!("Antigravity CLI failed: {e}")),
        }
    }
}
...
mod tests {
    ...
    fn test_missing_cli() {
        ...
        #[cfg(not(windows))]
        {
            let res = run_cmd_conpty(&PathBuf::from("/non/existent/agy"), &["--print", "/usage"], None, Duration::from_secs(5));
            assert!(res.is_err());
        }
    }
    ...
    #[cfg(unix)]
    #[test]
    fn unix_pipes_capture_echo() {
        let echo = PathBuf::from("/bin/echo");
        if !echo.is_file() {
            return;
        }
        let out = run_cmd_conpty(&echo, &["hello from unix runner"], None, Duration::from_secs(5))
            .expect("echo");
        assert!(out.contains("hello from unix runner"));
    }

...
/// Candidates in order: the native binary inside the global npm package (cleanest — no cmd/node
/// wrapper) → ~/.codex/bin → codex / codex.exe / codex.cmd on PATH.
    ...
    if let Some(appdata) = dirs::config_dir() {
        ...
        if let Ok(rd) = std::fs::read_dir(pkg.join("bin")) {
            for e in rd.flatten() {
                ...
                if n.starts_with("codex-") && n.contains("linux") && !n.contains('.') {
                    cands.push(e.path());
                }
            }
        }
        if let Ok(rd) = std::fs::read_dir(pkg.join("vendor")) {
            // Newer packages keep the native binary at vendor/<triple>/codex/codex(.exe)
            for e in rd.flatten() {
                let base = e.path().join("codex");
                cands.push(base.join("codex.exe"));
                cands.push(base.join("codex"));
            }
        }
        cands.push(appdata.join("npm").join("codex.cmd"));
        cands.push(appdata.join("npm").join("codex"));
    }
